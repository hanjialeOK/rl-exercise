...
class PPOAgent(BaseAgent):
    ...
    def _build_train_op(self):
        ...
        for i in range(self.k):
            ...
            # mu1_1 = self.actor1(ob1_ph)
            # mu1_2 = self.actor2(ob1_ph)
        # score1_1 = self.discriminator(ob1_ph, mu1_1)
        # score1_2 = self.discriminator(ob1_ph, mu1_2)
        # prop1 = tf.nn.softmax(tf.stack([score1_1, score1_2]), axis=0)
        prop1 = tf.nn.softmax(tf.stack(score1s), axis=0)
        prop1 = tf.expand_dims(prop1, axis=-1)
        # mu1 = tf.reduce_sum(prop1 * tf.stack([mu1_1, mu1_2]), axis=0)
        ...
        # mu_1 = self.actor1(obs_ph)
        # mu_2 = self.actor2(obs_ph)
        # score_1 = self.discriminator(obs_ph, mu_1)
        # score_2 = self.discriminator(obs_ph, mu_2)
        # prop = tf.nn.softmax(tf.stack([score_1, score_2]), axis=0)
        prop = tf.nn.softmax(tf.stack(scores), axis=0)
        prop = tf.stop_gradient(tf.expand_dims(prop, axis=-1))
        # mu = tf.reduce_sum(prop * tf.stack([mu_1, mu_2]), axis=0)

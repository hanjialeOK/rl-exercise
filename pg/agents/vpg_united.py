...
class VPGAgent(BaseAgent):
    ...
    def __init__(self, sess, obs_dim, act_dim, num_env=1,
                 ...
                 vf_coef=0.5, max_grad_norm=0.5, horizon=1024,
                 minibatch=64, gamma=0.99, lam=0.95,
                 target_kl=0.01, grad_clip=True):
        self.sess = sess
    ...
    def update(self, frac):
        ...
        for i in range(self.train_iters):
            ...
            for start in range(0, self.horizon, self.minibatch):
                ...
                kl_buf.append(kl)
                isgradclipped_buf.append(is_gradclipped)
                lr_buf.append(self.lr)

        mean_kl = np.mean(kl_buf)
        if mean_kl > self.target_kl * 1.5:
            self.lr /= 1.5
        elif mean_kl < self.target_kl / 1.5:
            self.lr *= 1.5
        # self.lr = np.clip(self.lr, 0, 3e-4)

        return [np.mean(pi_loss_buf), np.mean(vf_loss_buf),
                np.mean(entropy_buf), np.mean(kl_buf),
                np.mean(isgradclipped_buf), np.mean(lr_buf)]

...
def hessian_vector_product(f, params):
    ...
    x = tf.placeholder(shape=g.shape, dtype=tf.float32, name='x_ph')
...
def mlp_categorical_policy(x, ac, hidden_sizes, activation, output_action, action_space):
    act_dim = action_space.n
    logits = mlp(x, list(hidden_sizes)+[act_dim], activation, None)
    # Log_softmax helps fix overflow and underflow
    logp_all = tf.nn.log_softmax(logits)
    pi = tf.squeeze(tf.multinomial(logits, 1), axis=1)
    logp = tf.reduce_sum(tf.one_hot(ac, depth=act_dim) * logp_all, axis=1)
    logp_pi = tf.reduce_sum(tf.one_hot(pi, depth=act_dim) * logp_all, axis=1)

    old_logp_all_ph = tf.placeholder(
        shape=[None, act_dim], dtype=tf.float32, name='old_logp_all_ph')
    d_kl = categorical_kl(logp_all, old_logp_all_ph)

    return pi, logp, logp_pi, logp_all, old_logp_all_ph, d_kl


def mlp_gaussian_policy(x, ac, hidden_sizes, activation, output_action, action_space):
    act_dim = ac.shape.as_list()[-1]
    mu = mlp(x, list(hidden_sizes)+[act_dim], activation, output_action)
    log_std = tf.get_variable(
        name='log_std', initializer=-0.5*np.ones(act_dim, dtype=np.float32))
    std = tf.exp(log_std)
    pi = mu + tf.random_normal(tf.shape(mu)) * std
    logp = gaussian_likelihood(ac, mu, log_std)
    logp_pi = gaussian_likelihood(pi, mu, log_std)

    old_mu_ph = tf.placeholder(
        shape=(None, ) + (act_dim, ), dtype=tf.float32, name='old_mu_ph')
    old_log_std_ph = tf.placeholder(
        shape=(None, ) + (act_dim, ), dtype=tf.float32, name='old_log_std_ph')
    d_kl = diagonal_gaussian_kl(mu, log_std, old_mu_ph, old_log_std_ph)

    return pi, logp, logp_pi, mu, log_std, old_mu_ph, old_log_std_ph, d_kl
...
def mlp_actor_critic(x, a, hidden_sizes=(64, 64), activation=tf.tanh,
   ...
   # default policy builder depends on action space
    ...
    with tf.variable_scope('pi'):
        policy_outs = policy(x, a, hidden_sizes, activation,
                             output_activation, action_space)
        pi, logp, logp_pi, mu, log_std, old_mu_ph, old_log_std_ph, d_kl = policy_outs
    with tf.variable_scope('v'):
        v = tf.squeeze(
            mlp(x, list(hidden_sizes) + [1], activation, None), axis=1)
    return pi, logp, logp_pi, mu, log_std, old_mu_ph, old_log_std_ph, d_kl, v

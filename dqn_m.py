...
class NatureDQNNetwork(tf.keras.Model):
	...
	def __init__(self, num_actions, name=None):
		...
		self.conv1 = tf.keras.layers.Conv2D(32, [8, 8], strides=4, padding='same',
			  data_format='channels_first', activation=activation_fn, name='Conv')
...
class Agent():
	def __init__(self,
		...
		self.optimizer = tf.train.RMSPropOptimizer(learning_rate=0.00025, decay=0.95, \
	...
	def _build_networks(self):
		...
		self.online_net_output = self.online_network(self.state_ph)
		self.target_net_output = self.target_network(self.next_state_ph)
		self.q_argmax = tf.argmax(self.online_net_output, axis=1)[0]
		self.next_q_max = tf.reduce_max(self.target_net_output, axis=1) # (32, 1)
		if self.summary_writer is not None:
	...
	def _build_train_op(self):
		...
		next_q_max = tf.squeeze(self.next_q_max) # (32, )
		...
		q_value_chosen = tf.squeeze(
						     tf.gather(self.online_net_output,
							 	tf.expand_dims(self.actions_ph, axis=-1), axis=1, batch_dims=1))
...
def main(num_episodes=10000):
	...
	results_dir = os.path.abspath("/data/hanjl/my_results/{}".format(env.spec.id + "-" + \
										time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime(time.time()))))
	summary_dir = os.path.join(results_dir, "tf1_summaries")
	...
	with tf.Session() as sess:
		...
		for i in range(num_episodes):
			...
			episode_summary.value.add(simple_value=episode_rewards[i], node_name="episode_reward", tag="reward")
			episode_summary.value.add(simple_value=episode_lengths[i], node_name="episode_length", tag="length")
